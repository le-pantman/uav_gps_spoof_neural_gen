...
double spoof_lat = 47.397742; // 37.769
double spoof_lon = 8.545594;  // -119.5978
double spoof_alt = 488.0;     // 2134.0
...
void GpsPlugin::OnWorldUpdate(const common::UpdateInfo& /*_info*/)
{
  ...
  if (((int) current_time.Double()) % 1200 < 900) {
    ...
    gps_msg.set_latitude_deg(spoof_lat);
    gps_msg.set_longitude_deg(spoof_lon);
    gps_msg.set_altitude(spoof_alt);
  }
